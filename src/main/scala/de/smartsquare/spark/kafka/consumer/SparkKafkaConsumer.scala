...
class SparkKafkaConsumer {
  ...
  //props.put("bootstrap.servers", "194.95.79.98:9092")
  ...
  private val topic = kafkaTopic(0)
  ...
  private def readBatchFromTopic(topic: String, iterator: ConsumerIterator[Array[Byte], Array[Byte]]) = {
    ...
    if (batch.isEmpty) throw new IllegalArgumentException(s"$topic is  empty")else{ CassandraOperation.insertTweets(batch,batchSize)}
...
case object GiveMeWorkSpark

class SparkKafkaMessageConsumer(consumer: SparkKafkaConsumer) extends Actor  {
  ...
  context.system.scheduler.schedule(initialDelay, interval, self, GiveMeWorkSpark)

  def receive: PartialFunction[Any, Unit] = {

    case GiveMeWorkSpark => consumer.read
  }
...
object SparkKafkaConsumer extends App {
  ...
  val consumer = actorSystem.actorOf(Props(new SparkKafkaMessageConsumer(new SparkKafkaConsumer)))

  consumer ! GiveMeWorkSpark
